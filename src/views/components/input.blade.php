...
@switch($attributes->get('type'))
        ...
        @if ($multiple)
            ...
            >
                <div x-init="loadOptions()">
                    <input x-ref="{{ $id }}" id="{{ $id }}" type="hidden" x-bind:value="selectedValues()" {{ $attributes->merge() }}>
                                            ...
                                            <div
                                                ...
                                                <div class="flex flex-row-reverse flex-auto">
                                                    <div x-on:click.stop="remove(index,option)">
                                                        <svg class="w-4 h-4 fill-current " role="button"
                                                            viewBox="0 0 20 20">
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex items-center justify-between w-full">
                                                        ...
                                                        <div x-show="option.selected">
                                                            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><!--! Font Awesome Pro 6.2.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><path d="M470.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L192 338.7 425.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"/></svg>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="relative flex flex-col items-center">
                            <div @click="open" class="w-full">
                                <div class="flex p-1 my-2 bg-white border border-gray-200 rounded-lg shadow-sm">
                                    ...
                                    <div
                                        class="flex items-center w-8 py-1 pl-2 pr-1 text-gray-300">
                                        <button type="button" x-show="isOpen() === true" @click="open"
                                            class="w-6 h-6 text-gray-600 outline-none cursor-pointer focus:outline-none">
                                            <svg class="w-3 h-3 fill-current" viewBox="0 0 20 20">
                                                <path
    ...
    @case('toggle')
            ...
            x-data=
                "{ toggle:
                    @if ($attributes->whereStartsWith('wire:model')->first())
                        @entangle($attributes->whereStartsWith('wire:model')->first())
                    @else
                        false
                    @endif
                }"
        ...
        >
            <div class="relative inline-block w-8 h-4 rounded-full cursor-pointer">
                <input
                    class="absolute w-8 h-4 transition-colors duration-300 rounded-full appearance-none cursor-pointer peer bg-blue-gray-100 checked:bg-pink-500 peer-checked:border-pink-500 peer-checked:before:bg-pink-500 {{ $attributes->get('class') }}"
                    type="checkbox" id="{{ $id }}"
                    data-attribute="toggle"
                    {{ $attributes->merge() }} @click="toggle = !toggle" />
                <label
                    for="{{ $id }}"
                    class="before:content[''] absolute top-2/4 -left-1 h-5 w-5 -translate-y-2/4 cursor-pointer rounded-full border border-blue-gray-100 bg-white shadow-md transition-all duration-300 before:absolute before:top-2/4 before:left-2/4 before:block before:h-10 before:w-10 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity hover:before:opacity-10 peer-checked:translate-x-full peer-checked:border-pink-500 peer-checked:before:bg-pink-500 {{ $attributes->get('labelClass') }}"
                >
                ...
                <div class="w-64 mb-5">
                    ...
                    <div class="relative">
                        ...
                        <template x-if="showDatepicker">
                            <div class="absolute top-0 left-0 z-50 p-2 mt-12 bg-white rounded-lg shadow"
                                style="width: 17rem">
                                ...
                                <div>
                                    ...
                                    class="text-xl font-light text-gray-800"></span>
                                        ...
                                        <div style="width: 14.28%" class="px-1 mb-1">
                                            <div @click="getDateValue(date)" x-text="date"
                                                ...
                                                :class="{
                                                    ...
                                                    'text-[#393939] hover:bg-blue-600 hover:bg-opacity-25': isToday(date) == false && isSelectedDate(date) == false,
                                                    'bg-[#e91e63] text-white hover:bg-opacity-75': isSelectedDate(date) == true 
                                                }">
                                            </div>
                                        </div>
                                    </template>
    ...
    @case('timepicker')
        ...
        >
            <div class="w-fit" @click.away="close()">
                <input x-ref="{{ $id }}" id="{{ $id }}" class="px-1 py-2 text-gray-800 bg-transparent bg-white border border-gray-200 rounded-lg shadow-sm outline-none appearance-none cursor-pointer hover:bg-grey-100" :value="timepickerValue" @click="showTimepicker = !showTimepicker" />
                <template x-if="showTimepicker">
                    <div class="relative block py-1 w-fit">
                        <div class="flex relative rounded-lg shadow-lg p-0.5 border border-gray-200">
                            <input type="number" min="1" max="12" step="1" class="ml-3 text-center" x-model="selectedHour" @click="update()" />

                            <span class="px-1 py-2 text-gray-800 outline-none appearance-none">:</span>

                            <input type="number" min="0" max="59" step="5" class="text-center" x-model="selectedMinute" @click="update()" />

                            <div @click="changeAmPm()" class="px-1 py-2 mr-3 text-gray-800 uppercase bg-transparent bg-white outline-none appearance-none cursor-pointer hover:bg-grey-100" x-text="selectedAmPm"></div>
                        </div>
                    </div>
                </template>
            </div>
    ...
    @case('number')
        <div wire:ignore class="relative h-11 w-full min-w-[200px]">
            <input
                @class([
                    "peer h-full w-full border-b border-blue-gray-200 bg-transparent pt-4 pb-1.5 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border-blue-gray-200 focus:border-pink-500 focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50" => ($inputType != 'outline'),
    ...
    @case('counter')
        ...
        >
            <button class="w-6 h-6 flex items-center justify-center border rounded-full p-4 text-2xl hover:text-white hover:bg-[#e91e63] transition mr-3" @click="counter--">
                –
            </button>

            <input class="w-1/3 py-3 pl-4 pr-10 font-medium leading-none text-gray-600 rounded-lg shadow-sm focus:outline-none focus:ring focus:ring-blue-600 focus:ring-opacity-50" type="number" id="{{ $id }}" :value="counter" {{ $attributes->merge() }}  />
            
            <button class="w-6 h-6 flex items-center justify-center border rounded-full p-4 text-2xl hover:text-white hover:bg-[#e91e63] transition ml-3" @click="counter++">
                +
            </button>
        </div>
    @break
    ...
    @case('file')
        <input class="bg-white min-h-[150px] rounded-lg w-full p-5" id="{{ $id }}" type="file" {{ $attributes->merge() }} />
    @break
    ...
    @case('range')
        ...
        @endif
        <input id="{{ $id }}" type="range" class="cursor-pointer h-[3px] border-none border-transparent " {{ $attributes->merge() }} />
    @break
            ...
            >
                <input
                    ...
                    @class([
                        ...
                        $color ? $color : 'checked:bg-pink-500 checked:before:bg-pink-500',
                        $border ? $border : 'checked:border-pink-500',
                        $attributes->get('class'),
                    ])
                    {{ $attributes->merge() }}
                />
            ...
            >
                <input
                    @class([
                        ...
                        $color ? $color : 'checked:before:bg-pink-500',
                        $border ? $border : 'checked:border-pink-500',
                        $textColor ? $textColor : 'text-pink-500',
                        $attributes->get('class'),
                    ])
                    type="radio"
                    id="{{ $id }}"
